...
const ROUTES = [
  { method: "POST", pattern: "/posts", resource: "posts", fields: ["title", "body"], handler: function(D,V,req){ const m=V.missing(req.body, req.fields); if(m) return {status:400, body:{error:'missing '+m}}; const row=D.insert(req.resource, req.body); return {status:201, body:row}; } },
  { method: "GET", pattern: "/posts", resource: "posts", fields: ["title", "body"], handler: function(D,V,req){ return {status:200, body:D.all(req.resource)}; } },
  { method: "GET", pattern: "/posts/:id", resource: "posts", fields: ["title", "body"], handler: function(D,V,req){ const row=D.find(req.resource, req.params.id); if(!row) return {status:404, body:{error:'not found'}}; return {status:200, body:row}; } },
  { method: "PUT", pattern: "/posts/:id", resource: "posts", fields: ["title", "body"], handler: function(D,V,req){ const cur=D.find(req.resource, req.params.id); if(!cur) return {status:404, body:{error:'not found'}}; const upd=D.update(req.resource, req.params.id, req.body); return {status:200, body:(upd||cur)}; } },
  { method: "DELETE", pattern: "/posts/:id", resource: "posts", fields: ["title", "body"], handler: function(D,V,req){ const cur=D.find(req.resource, req.params.id); if(!cur) return {status:404, body:{error:'not found'}}; D.remove(req.resource, req.params.id); return {status:204, body:null}; } },
